...
import { persistReport as archivePersist, KIND_SCHEMA_VERSION } from './archive.js';
...
// ⚠ 这是**契约顺序**（validatePhases 返回值 / 面板勾选项顺序 / 报告 phases 字段），
//   不等于**执行顺序**——见下方 PHASE_EXEC_ORDER。
export const MODEL_TEST_PHASES = ['probe', 'rpm', 'context', 'quota-group'];

// v0.9.8.1：**执行顺序**。与契约顺序的唯一差异是 context 提到 rpm 之前。
//
// 原因：rpm 相是最多 4 档 × 5 样本的阶梯压测，会大量消耗上游的 TPM（tokens per minute）
// 额度。若 context 相排在其后，测到的「能否接受 N tokens 输入」会被前序 TPM 消耗污染——
// 实机 ST-rrx/ST-rrx199 的 deepseek-v4-flash 就出现过 1024 通过、8192 报 RATE_LIMIT，
// 而上游错误消息是 `inference exceeds tpm/rpm limit`（TPM 类），并非「输入过长」。
//
// 隔离后：context 相在 TPM 未被消耗时执行，测出的容量上限才干净。rpm 相放最后不受影响
// （它本身就是「压测到限流」的相，越到后面越接近真实边界）。
//
// 短路语义随之变化（更保守且更合理）：context 相拿到 CONTEXT_LENGTH 类错误时，
// 现在会同时跳过 rpm 与 quota-group——模型容量都不够，测速率无意义。
export const PHASE_EXEC_ORDER = ['probe', 'context', 'rpm', 'quota-group'];
...
export function validatePhases(v) {
  ...
  // 去重 + 固定序（契约顺序，供展示与契约消费）
  return MODEL_TEST_PHASES.filter((p) => v.includes(p));
}

/** 把「已选相集合」按**执行顺序**排列。供 _runTarget 遍历与报告标注。 */
export function orderPhasesForExecution(phases) {
  return PHASE_EXEC_ORDER.filter((p) => phases.includes(p));
}
...
export class ModelTestRunner {
  /**
   * @param {object} opts
   * @param {string|null} [opts.reportDir] 档案目录（由 `index.js` 用
   *   `archive.resolveArchiveDir(cfg)` 注入）。**与 `reports.enabled` 无关**——
   *   档案落盘是旁路功能，不该被「每日报告」开关连坐（v0.9.20 修复 C1）。
   * @param {object|null} [opts.daily] 日账本（**仅作 reportDir 兜底**，兼容旧调用方）
   */
  constructor({ llm, registry, daily, log, prompt = 'ping', timeoutMs = 20000, threshold, reportDir = null }) {
    this.llm = llm;
    this.registry = registry;
    this.daily = daily; // 仅用于 reportDir 兜底（旧调用方未传 reportDir 时）
    this.log = log;
    this.prompt = prompt;
    ...
    this.last = null;
    /** v0.9.20：显式注入的档案目录（优先于 daily 兜底） */
    this.reportDir_ = reportDir;
  }

  /**
   * 档案目录。**优先显式注入**（`reportDir_`，与 probe/loadtest 同源），
   * 其次回退 `daily.reportDir`（兼容旧调用方）。
   *
   * v0.9.20 修复 C1：此前只读 `daily.reportDir`，而 `daily` 仅在
   * `cfg.reports.enabled === true` 时创建 → **默认配置下 model-test 档案静默不落盘**，
   * 与 v0.9.9.3 声明的「档案落盘不依赖 reports.enabled」矛盾。
   */
  get reportDir() {
    if (this.reportDir_) return this.reportDir_;
  ...
  async run(targets, opts = {}) {
    ...
    const report = {
      ...
      partial: false,
      phases,
      phaseOrder: orderPhasesForExecution(phases),
      targets: [],
    };
    try {
      for (const t of targets) {
  ...
  async _runTarget(t, recoveryMs, phases) {
    ...
    // v0.9.8.1：按**执行顺序**遍历（context 提到 rpm 之前），使 context 相不受 rpm 相的
    // TPM 消耗污染。phases / notSelected 仍按契约顺序，供面板与报告展示。
    const execOrder = orderPhasesForExecution(phases);
    const outcome = {
      aborted: false,
      _sawThrottle: false,
      startedAt: new Date(tStart).toISOString(),
      phases: phases.slice(),
      phaseOrder: execOrder.slice(),
  ...
  _finalize(t, outcome) {
    const {
      ...
      phases, phaseOrder, notSelected, skipped, skipReasons, phaseErrors,
    } = outcome;
    const targetResult = {
      provider: t.provider,
      model: t.model,
      ...
      phaseOrder: phaseOrder ?? orderPhasesForExecution(phases ?? MODEL_TEST_PHASES),
      notSelected: notSelected ?? [],
      // v0.9.8.1：skipped 按**契约顺序**规范化，与 phases / notSelected 三者同序，便于对照展示。
      // （收集时按执行序 push，此处重排；skipReasons 是对象，顺序无关。）
      skipped: MODEL_TEST_PHASES.filter((p) => (skipped ?? []).includes(p)),
...
export function formatReportMarkdown(report) {
  ...
  // v0.9.8.1：执行顺序与契约顺序不同时显式标注，避免「报告里的相顺序与勾选顺序不一致」被误读为 bug。
  if (Array.isArray(report.phaseOrder)) {
    const sameAsContract = Array.isArray(report.phases)
      && report.phaseOrder.join(',') === report.phases.join(',');
    lines.push(`- 执行顺序：${report.phaseOrder.join(' → ')}`
      + (sameAsContract ? '' : '（context 相提前至 rpm 相之前，避免 rpm 的 TPM 消耗污染上下文容量测量）'));
  }
  lines.push('');
  lines.push('## 逐模型结论');
  lines.push('');
 ...
 * @param {object} opts { llm, registry, daily, log, targets, recoveryMs, prompt,
 *   timeoutMs, confirmPaidBurn, threshold, testThreshold, reportDir }
 *   `reportDir`：显式档案目录（**优先**，与 probe/loadtest 同源）。
 *   未传时回退 `daily.reportDir`（兼容旧调用方）——**注意 daily 仅在
 *   `reports.enabled=true` 时存在**，故新调用方应显式传 `reportDir`（v0.9.20 修复 C1）。
...
export async function runReport(opts) {
  ...
  const runner = new ModelTestRunner({
    ...
    prompt: opts.prompt, timeoutMs: opts.timeoutMs, threshold,
  ...
  report.runId = runId;
  // v0.9.20 修复 C1：统一走 runner.reportDir（显式注入优先 → daily 兜底），
  // 不再直接依赖 daily —— 后者仅在 reports.enabled=true 时存在。
  const dir = runner.reportDir;
  if (dir) {
    persistReport(dir, runId, report, log);
  } else {
    log?.warn?.('model-test: 档案目录不可用，跳过落盘');
  }
  return report;
}
function buildRunId() {
 ...
 *
 * v0.9.9.1：顶层加 `kind: 'model-test'` 与 `schemaVersion: 1`（档案统一 schema）；
 * 原子写与命名复用 `lib/archive.js`（消除与 routes.js 的重复实现）。
 * 向后兼容：旧档案无 `kind` 时，读取端（`listRunJson` / `/test-archives`）按 'model-test' 处理。
 */
function persistReport(reportDir, runId, report, log) {
  try {
    // 规范化 kind/schemaVersion（不信任调用方，落盘前统一）
    report.kind = 'model-test';
    report.schemaVersion = KIND_SCHEMA_VERSION;
    const result = archivePersist(reportDir, {
      runId,
      kind: 'model-test',
      jsonBody: JSON.stringify({ ...report, timeZone: null }, null, 2),
      mdBody: formatReportMarkdown({ ...report, timeZone: null }),
    }, log);
    if (!result) return null;
    // 回置绝对路径（既有两个字段，保持契约不变）
    report.file = result.jsonTarget;
    report.md = result.mdTarget;
    return result.jsonTarget;
  } catch (error) {
