...
import { persistReport, KIND_SCHEMA_VERSION } from './archive.js';
...
/**
 * 校验负载测试入参（v0.9.3 S-3 加固）。
 * - 归一化 tiers：缺省即 ['free']；过滤非字符串项。
 * - 含非 free tier（付费档）时，必须显式 confirmPaidBurn=true，否则拒绝，
 *   防止误操作真实烧付费 token / 触发上游限流。
 * @param {object} [opts] 原始入参（含 tiers / confirmPaidBurn 及透传字段）
 * @returns {object} 规范化后的 opts（tiers 已归一化，其余字段原样透传）
 * @throws {Error} 付费 tier 缺确认时拒绝
 */
export function validateLoadtestOptions(opts = {}) {
  const rawTiers = Array.isArray(opts.tiers) && opts.tiers.length
    ? opts.tiers.filter((t) => typeof t === 'string' && t)
    : DEFAULT_TIERS;
  const hasNonFree = rawTiers.some((t) => t !== 'free');
  if (hasNonFree && opts.confirmPaidBurn !== true) {
    throw new Error('非 free tier 会真实消耗付费 token；请在 body 中显式声明 "confirmPaidBurn": true 后再试');
  }
  return { ...opts, tiers: rawTiers };
}
...
/**
 * v0.9.9.2 Task 3：生成 loadtest 档案 runId。
 * 格式与 model-test 一致（`<YYYY-MM-DD>T<HH-MM-SS>-<rand4>`），便于统一 lister 排序。
 */
function buildLoadtestRunId() {
  const now = new Date();
...
export class LoadTestRunner {
   ...
   */
  constructor({ llm, registry, log, prompt = 'ping', timeoutMs = 20000, reportDir = null }) {
    this.llm = llm;
    ...
    this.running = false;
    /**
     * v0.9.9.2 Task 3：档案落盘目录（null = 不落盘）。
     * 由 index.js 用 `archive.resolveArchiveDir(cfg)` 注入——**不依赖 reports.enabled**。
     */
    this.reportDir = reportDir;
    /**
     * v0.9.9.2 Task 3：实例级 runId（一份「累积多 phase」的完整负载测试报告）。
     * 每次 phase 完成覆盖写同一档案——loadtest 是用户逐 phase 手动触发的，
     * 累积成一份比每 phase 一份更符合直觉。
     */
    this.runId = buildLoadtestRunId();
    this._archiveStartedAtMs = Date.now();
    this.aborted = false;
    this.startedAt = null;
  ...
  async run(phase, opts = {}) {
    ...
    try {
      ...
      // v0.9.9.2 Task 3：每个 phase 跑完落盘一次（覆盖同 runId）。
      // 放在 finally：即使 phase 抛错也落盘已完成的 phase 结果（诊断价值）。
      this._persistArchive();
    }
  }

  /**
   * 落盘当前累积结果（v0.9.9.2 Task 3）。
   *
   * - **覆盖写**：同 runId，每次 phase 完成后覆盖（tmp + rename 原子替换）
   * - **失败不阻塞**：`persistReport` 内部 catch + warn，不影响跑批返回
   * - `reportDir` 为 null 时静默跳过（未注入目录）
   */
  _persistArchive() {
    if (!this.reportDir) return;
    const finishedAtMs = this.finishedAt ?? Date.now();
    const body = {
      kind: 'loadtest',
      schemaVersion: KIND_SCHEMA_VERSION,
      runId: this.runId,
      startedAt: new Date(this._archiveStartedAtMs).toISOString(),
      finishedAt: new Date(finishedAtMs).toISOString(),
      elapsedMs: finishedAtMs - this._archiveStartedAtMs,
      aborted: this.aborted === true,
      // 已完成的 phase 列表 + 各自结果（按完成顺序）
      phases: Object.keys(this.results),
      results: { ...this.results },
    };
    persistReport(this.reportDir, {
      runId: this.runId,
      kind: 'loadtest',
