 ...
 * 能力（v1.0.2）：
 *
 * 1. **规则路由**：`rules` 声明候选链，支持四种候选扩展策略（explicit / same-model /
 *    same-provider / exclude-current，源自 dsh 模型注册表）。
 *    **v0.9.1 起为「纯选择驱动」**：规则 = 命名「规则包」，在对话框模型选择器选中即
 *    完整接管（按链首选发送 + 报错/超时沿链切换 + 应用该包 mode/custom 参数）；
 *    不选任何包 = 透传直连、插件不介入。`rules` 支持面板热更新（store JSON 持久化，
 *    免改 patch 免重启）。`propose` 为 v0.9.1 **之前的遗留兼容字段** —— 新客户端不再
 *    提交，也不参与路由（保留仅为兼容旧客户端/日志）。
 *
 * 2. **无感故障切换**：llm/stream 包装层 —— finish 分片驱动 + **commit-on-substantive**
 *    （v0.6.0 修订，替代 v0.5 的 commit-on-first-chunk）+ TTFT 看门狗 + cooldown 熔断
 *    + 候选耗尽合成 error finish（不 throw）。
 *    实现见 `lib/wrapper/index.js`（`lib/wrapper.js` 仅为 20 行 re-export 薄桥）。
 *
 * 3. **用量记账与限额**：usage 分片按 provider 记滚动 5h/1w 窗口；渠道可经
 *    `providerMeta` 声明窗口限额（5h / 1周 / 自定义）与重置规则；free-tier 另受
 *    跨请求节流约束。
 *
 * 4. **每日 API 报告**：wrapper 的透传/切换调用均记账，每日 `reports.hour` 自动生成
 *    昨日 Markdown 报告；关闭则零记账零调度（`reports.enabled=false`）。
 *
 * 5. **模型测试与档案**：模型全自动测试（独立 phase + 退避 + 短路）；三类档案
 *    （model-test / loadtest / probe）统一列表与详情。
 *
 * 6. **峰谷段场景**：`timeWindows` 按 `timeZone` 分段，peak / valley 各自候选链。
 *
 * 7. **WebUI 面板**（8 页签）+ 状态 / 状态写回 / 档案 / 报告等接口（回环围栏）。
 *
...
import { loadState, saveState, makeSaveStateFn } from './store.js';
...
export function apply(ctx, config) {
  ...
  // v0.9.9.2 Task 4：注入档案目录（不依赖 reports.enabled——那是「每日报告」开关）
  const archiveDir = resolveArchiveDir(cfg);
  const probe = new ProbeBoard(log, { ...cfg.probe, reportDir: archiveDir });
  const metrics = new Metrics();
  // v0.9.10 Task 2：metrics 注入 Router（节流判定需要近 60s 请求计数）
  const router = new Router(cfg, registry, probe, metrics);
  ...
  // v0.9.9.2 Task 3：注入档案目录（复用上面算好的 archiveDir）。
  const loadtest = new LoadTestRunner({ llm, registry, log, reportDir: archiveDir });
  ...
  const modelTest = new ModelTestRunner({
    ...
    // v0.9.20 修复 C1：显式注入 archiveDir（与 probe/loadtest 同源）。
    // 此前只传 daily → daily 仅在 reports.enabled=true 时创建 → **默认配置下
    // model-test 档案静默不落盘**，与 v0.9.9.3「档案落盘不依赖 reports.enabled」
    // 的声明矛盾。daily 保留作旧调用方兜底。
    reportDir: archiveDir,
  });
  log.info(`model-test ready: reportDir=${modelTest.reportDir ?? '(不可用)'}`);
  ...
  const routeDisposers = makeStatusRoutes({
    ...
    // v1.0.1 修复（**Critical · 数据丢失**）：`/state` 保存须**合并**已有持久化状态，
    // 而非整体替换。
    //
    // **原缺陷**：直接 `saveState(cfg.storePath, state, log)` —— 写的是「本次请求派生的
    // state」。`normalizeState` 对未提交字段返回 `undefined`，而 `JSON.stringify` **丢弃
    // undefined** → 磁盘上已有的值被**静默擦除**。
    //
    // **真实触发（面板 4 个保存点全部命中）**：
    //   · 「一键启用每日报告」只发 `{reports}` → rules / providerMeta / timeZone 全丢
    //   · 「保存规则」只发 `{rules}`        → providerMeta / reports / timeZone / mode 全丢
    //   · 「exclude 操作」发 `{rules, providerMeta}` → reports / timeZone / mode 全丢
    //   且响应为 `200「已保存并即时生效」` —— **静默**数据丢失。
    //
    // **语义对齐**：内存侧本就是「未提交则保留」（见 `routes.js` 对 providerMeta /
    // reports / timeWindows 的 `!== undefined` 判断），磁盘侧必须与之一致。
    //
    // 注：修复放在此处（注入点）而非 4 个调用点 —— 一处覆盖全部，且调用方无需感知。
    saveStateFn: makeSaveStateFn(cfg.storePath, log),
    daily,
    reporter,
    loadtest,
    modelTest,
