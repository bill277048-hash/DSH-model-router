 ...
 *
 * F-3（批 5）：虚拟 model-router provider（PKG_PROVIDER）被排除在 provider 采集
 * （refreshProviders）与 registeredPairs 之外——它只是 UI 展示用的虚拟分组，
 * 不应进入候选展开（否则把 `__pkg:*` 当真实候选，路由语义崩坏）。
 */

import { PKG_PROVIDER } from './router.js';
...
export class Registry {
  ...
  /**
   * v0.9.6：provider 是否被声明排除（providerMeta.<provider>.exclude === true）。
   *
   * 语义边界（刻意收窄）：exclude 只裁剪「注册表自动展开」的候选池，即
   * registeredPairs() 的消费者（same-model / same-provider / exclude-current 三策略）。
   * **不否决**下列「用户显式选择」路径：
   *   - router.expandByStrategy 的 explicit 分支（直接返回 rule.route 手工列表）
   *   - wrapper 的首选 seed（用户当前会话正在用的模型）
   *   - router.resolvePackage / pickPrimary
   * 理由：面板「排除」按钮的语境是 model-test verdict=exclude（「别再自动选它」），
   * 而非「全局禁用该 provider」；若连显式 route 也拦，会把当前请求直接打回透传，行为突兀。
   * @param {string} provider
   * @returns {boolean}
   */
  isExcluded(provider) {
    return this.config?.providerMeta?.[provider]?.exclude === true;
  }
  ...
  refreshProviders() {
    try {
      ...
      const providers = [];
      // F-3（批 5）：跳过虚拟 provider（PKG_PROVIDER）——只参与 UI 展示，不进真实候选集。
      for (const provider of this.llm.adapters.keys()) {
        if (provider === PKG_PROVIDER) continue;
      ...
      // 目录里声明但未注册（休眠）的 provider 也列出，标注 dormant；虚拟 provider 同样排除
    ...
    try {
      for (const p of this.snapshot.providers) {
        ...
        try {
          ...
          // 供 router contextReorder 做窗口感知重排；无窗口时缺省。
  ...
  registeredPairs() {
    ...
    for (const p of this.snapshot.providers) {
      if (p.dormant) continue;
      // v0.9.6：被声明 exclude 的 provider 不进自动展开候选池（一处覆盖三策略：
      // same-model / same-provider / exclude-current 都经本函数取候选）。
      if (this.isExcluded(p.provider)) continue;
      const meta = this._profileMeta(p.provider);
      if (p.models.length === 0) {
        pairs.push({ provider: p.provider, model: null, ...(meta ?? {}) });
      } else {
        for (const m of p.models) {
          // v0.9.4（修正 1）：hop 带出 contextWindow（数字窗口缺省不携带）
          pairs.push({
            provider: p.provider,
            model: m.id,
            ...(meta ?? {}),
