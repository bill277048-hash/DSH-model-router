...
/**
 * v0.9.9.2 Task 4：生成 probe 档案 runId。
 * 格式与 model-test/loadtest 一致，便于统一 lister 排序。
 */
function buildProbeRunId() {
  const now = new Date();
 ...
 * @note 导出供 v0.8.0 B-1 loadtest 编排层复用（PROBE_MARK 透传 + 超时 + finish 解析）。
 */
export async function singleRaw(llm, provider, model, prompt, timeoutMs, log) {
  const started = nowMs();
      ...
      if (chunk?.type === 'finish') {
        ...
        if (kind === 'stop') {
          ...
          outcome = {
            ...
            // v0.9.8：保留上游真实错误消息，给那 4 个 400 的 case 留出排查线索。
            // 后续 _phaseProbe/_phaseContext 把它落进报告 md/JSON。
            errorMessage: chunk.reason?.failure?.message,
            errorKind: 'error',
          };
        } else if (kind === 'aborted') {
          outcome = { ok: false, errorCode: 'ABORTED', errorMessage: 'aborted', errorKind: 'aborted' };
        }
        finished = true;
...
export class ProbeBoard {
  ...
  constructor(log, opts = {}) {
    ...
    this.running = false;
    /**
     * v0.9.9.2 Task 4：档案落盘目录（null = 不落盘）。
     * 由 index.js 用 `archive.resolveArchiveDir(cfg)` 注入——**不依赖 reports.enabled**。
     */
    this.reportDir = opts.reportDir ?? null;
    /**
     * v0.9.9.2 Task 4：实例级 runId（一份「最新健康快照」档案，覆盖写）。
     * 探测是周期高频的（默认 300s 一轮），每轮一份会无界堆积磁盘；
     * 健康快照的价值在「当前状态」，故实例级 runId + 覆盖写。
     */
    this.runId = buildProbeRunId();
  ...
  _record(provider, model, rec) {
    ...
    h.lastErrorMessage = rec.ok ? undefined : rec.errorMessage;
  ...
  async runAll(llm, targets) {
    ...
    try {
      ...
      this.running = false;
      // v0.9.9.2 Task 4：一轮探测完成落盘（覆盖同 runId）。
      // 放在 finally：即使个别 target 抛错也落盘已探到的部分（诊断价值）。
      this._persistArchive(targets);
    }
  }

  /**
   * 落盘当前健康快照（v0.9.9.2 Task 4）。
   *
   * - **覆盖写**：同 runId（实例级），每轮探测完成后覆盖
   * - **只落 json**：probe 是快照数据，md 价值低（方案 Open Question 3）
   * - **失败不阻塞**：`persistReport` 内部 catch + warn
   */
  _persistArchive(targets) {
    ...
    const body = {
      ...
      elapsedMs: nowMs - this._archiveStartedAtMs,
