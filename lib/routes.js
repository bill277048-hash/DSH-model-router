...
/**
 * v0.9.23：`listRunJson` 的**重访阈值**。超过此档案数时该端点会主动 warn，
 * 提示应改为「轻量列表 + 点行惰性取详情」。
 *
 * 取值依据（v0.9.21 实测）：10 份档案 / 173KB → 12–17ms，**线性增长**；
 * 200 份线性外推约 300ms —— 该量级已值得改造，故阈值取 150 留出提前量。
 */
const LIST_REVISIT_THRESHOLD = 150;
let VERSION = '0.0.0';
try {
  ...
  return [
    ...
    },
    {
      ...
      handler: guard(async (req, res) => {
        ...
        // v1.0.1：传入**原始请求体的键集** —— normalizeState 会给未提交字段填默认值
        // （如 body 无 rules 时仍返回 rules: []），故合并必须依据「客户端实际提交了哪些键」。
        const submittedKeys = body && typeof body === 'object' && !Array.isArray(body)
          ? Object.keys(body)
          : [];
        const saved = saveStateFn(state, submittedKeys);
    ...
    },
    {
      ...
      handler: guard(async (req, res) => {
        ...
        const { normalizeState } = await import('./config.js');
        let state;
        // v1.0.1：本端点只提交 rules + providerMeta —— 显式记录「提交了哪些键」，
        // 供 saveStateFn 合并（否则 timeZone/mode/reports/timeWindows 会被擦除）。
        const patchBody = {
          rules: config.rules,
          providerMeta: { ...(config.providerMeta || {}), [provider]: newMeta },
        };
        try {
          state = normalizeState(patchBody, config.allowLegacyMatch === true, { log });
        } catch (error) {
          writeJson(res, 400, { ok: false, error: error.message });
          return;
        }
        ...
        const saved = saveStateFn(state, Object.keys(patchBody));
    ...
    },
    {
      ...
      handler: guard(async (req, res) => {
        // v0.9.24：移除 `!daily || !reporter` 门禁 —— **C1 的读取路径残留**。
        //
        // 历史：v0.9.9 前 model-test 借用 daily 的 reportDir，故端点要求 daily 存在。
        // v0.9.20（C1 修复）把**写入**路径解耦到 archiveDir（与 probe/loadtest 同源），
        // 但**读取**路径仍被这道门禁拦住 → 表现为：
        //   reports.enabled=false 时，档案**已正常落盘**（C1 已修），
        //   但 GET /model-test 返 503「需先启用每日报告」，
        //   用户只能从「测试档案」页签看到同一批档案 —— 两个页签行为不一致。
        //
        // 本处理器全程只用 `modelTest`（daily/reporter 仅在此门禁出现过，已 grep 确认），
        // 且同文件的 /model-test/list 与 /model-test/manual 早已只用 modelTest.reportDir。
        // 故统一为同一判据（与那两处对齐）。
        if (!modelTest || !modelTest.reportDir) {
          writeJson(res, 503, { ok: false, error: 'model-test 未装配或档案目录不可用' });
          return;
        }
        if (req.method === 'GET') {
 ...
 * v0.9.23 起该条件由 `LIST_REVISIT_THRESHOLD` 在运行时**主动告警**（越界才打）。
 *
...
function listRunJson(dir, log) {
  ...
  try {
    ...
    // v0.9.23：把「何时该重访」（见上方文档）从注释升级为**运行时信号**。
    // 本函数开销随档案数**线性增长**（每份都要读全文），到 ~200 份时约 300ms。
    // 与其依赖有人记得读注释，不如在越过阈值时主动 warn —— 且**只在越界时打**
    // （正常情况零日志噪音），并带上实测耗时供决策。
    const t0 = Date.now();
    ...
    // 越界才 warn（正常情况零噪音），带上实测耗时供决策
    if (out.length > LIST_REVISIT_THRESHOLD) {
      log?.warn?.(
        `model-test 档案数 ${out.length} 已超过重访阈值 ${LIST_REVISIT_THRESHOLD}`
        + `（本次 listRunJson 耗时 ${Date.now() - t0}ms）——`
        + `该端点开销随档案数线性增长，建议改为「轻量列表 + 点行惰性取详情」，`
        + `详见 lib/routes.js:listRunJson 文档。`
      );
    }
    return out;
  } catch {
