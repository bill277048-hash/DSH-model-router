...
import { persistReport as archivePersist, writeAtomic as archiveWriteAtomic, KIND_SCHEMA_VERSION, ARCHIVE_KINDS, archiveName, kindOfFilename } from './archive.js';
...
function serializeTimeWindows(tw, tz) {
  ...
  // 复用 router 的 segmentOf（避免重复实现）；routes 已经在 import chain 里
  // 改用独立小函数避免循环依赖——与 router.segmentOf 实现对齐（半开区间 [peak, valley)）
  const HHMM = (() => {
  ...
  const inPeak =
    (peakStart <= valleyStart)
      ...
      : (HHMM >= peakStart || HHMM < valleyStart);
  const segment = inPeak ? 'peak' : 'valley';
