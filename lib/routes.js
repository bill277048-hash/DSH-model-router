  ...
  return [
    ...
    },
    {
      ...
      handler: guard(async (req, res) => {
        ...
        const snap =
          quota && typeof quota.quotaSnapshot === 'function' ? quota.quotaSnapshot(provider) : null;
        writeJson(res, 200, {
          ok: true,
          provider,
    ...
    },
    {
      ...
      handler: guard(async (req, res) => {
        ...
        const opts = {
          ...
          prompt: typeof body.prompt === 'string' && body.prompt ? body.prompt : undefined,
          timeoutMs: typeof body.timeoutMs === 'number' ? body.timeoutMs : undefined,
        };
        void modelTest.run(targets, { recoveryMs: opts.recoveryMs, runId })
          .then((report) => {
        ...
        writeJson(res, 202, {
          ...
          willRun: true,
          message: `model-test 已启动（${targets.length} 个模型，串行 4 相；结果 GET 本端点或落盘报告目录）`,
        });
      }),
    },
    {
      ...
      try {
        ...
        meta = {
          ...
          targets: Array.isArray(raw.targets) ? raw.targets.map((t) => ({
            ...
            label: t.label ?? null,
            tier: t.tier ?? 'free',
            probe: t.probe ? { ok: t.probe.ok, errorCode: t.probe.errorCode ?? null } : null,
            rpm: t.rpm ? { lastOkRpm: t.rpm.lastOkRpm ?? null, first429Rpm: t.rpm.first429Rpm ?? null } : null,
            context: t.context ? { maxAccepted: t.context.maxAccepted ?? null } : null,
