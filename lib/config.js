...
export function normalizeTimeWindows(tw) {
  ...
  // route 项校验：复用 checkProviderModel（lib/config.js 已有），但参数未定——留待
  // normalizeConfig 内部或 routes.js 校验。本函数先校验结构。
  for (const seg of ['peak', 'valley']) {
    ...
    if (Array.isArray(segObj.route)) {
      for (let i = 0; i < segObj.route.length; i++) {
        const hop = segObj.route[i];
        if (!isPlainObject(hop) || typeof hop.provider !== 'string' || typeof hop.model !== 'string') {
          throw new Error(`model-router: timeWindows.${seg}.route[${i}] 须为 {provider, model}`);
        }
      }
    }
  }
  ...
  return Object.freeze({
    ...
    valleyStart,
    peak: Object.freeze({
      route: Object.freeze(Array.isArray(tw.peak?.route) ? tw.peak.route.slice() : []),
    }),
    valley: Object.freeze({
      route: Object.freeze(Array.isArray(tw.valley?.route) ? tw.valley.route.slice() : []),
    }),
  });
}
