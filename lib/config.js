...
/**
 * v0.9.9（方案 §6-1 第 3 条）：timeWindows 校验与规范化。
 *
 * 校验项：
 * - 必须是对象；未提交 → undefined（normalizeConfig 视为「保留 patch 现值」）
 * - peakStart / valleyStart 必须是 HH:MM 字符串（24 小时制、零填充）
 * - peakStart != valleyStart（对齐既有 hours 校验，半开区间拼接完整 24h）
 * - peak.route / valley.route 每项经 checkProviderModel 校验（不存在的 provider/model 直接报错）
 *
 * 返回值：冻结的规范化对象，**与 DEFAULT_CONFIG.timeWindows 深拷贝隔离**——避免
 * 浅拷贝引入的 mutation 风险（DEFAULT_CONFIG 自身已 freeze）。
 *
 * @param {object|undefined} tw
 * @returns {object|undefined} undefined 表示「未提交 / 无效，不热生效」
 */
export function normalizeTimeWindows(tw) {
  if (tw === undefined || tw === null) return undefined;
  if (!isPlainObject(tw)) {
    throw new Error('model-router: timeWindows 须为对象');
  }
  const HHMM = /^\d{2}:\d{2}$/;
  const peakStart = tw.peakStart;
  const valleyStart = tw.valleyStart;
  if (typeof peakStart !== 'string' || !HHMM.test(peakStart)) {
    throw new Error(`model-router: timeWindows.peakStart 须为 HH:MM 字符串（实际 ${JSON.stringify(peakStart)}）`);
  }
  if (typeof valleyStart !== 'string' || !HHMM.test(valleyStart)) {
    throw new Error(`model-router: timeWindows.valleyStart 须为 HH:MM 字符串（实际 ${JSON.stringify(valleyStart)}）`);
  }
  if (peakStart === valleyStart) {
    throw new Error('model-router: timeWindows.peakStart 与 valleyStart 不能相等（半开区间拼接完整 24 小时）');
  }
  // route 项校验：复用 checkProviderModel（L242，本文件内），与 rules[].route
  // 走同一函数，保证两条路径校验强度一致（v0.9.9.1 方案 §10 R1 修复）。
  // 该函数会拒空 provider/model，并提取 hop 上的 quotaGroup/tier/key 等
  // 内联字段——与 rules[].route 行为一致。
  for (const seg of ['peak', 'valley']) {
    const segObj = tw[seg];
    if (segObj === undefined || segObj === null) continue;
    if (!isPlainObject(segObj)) {
      throw new Error(`model-router: timeWindows.${seg} 须为对象（实际 ${JSON.stringify(segObj)}）`);
    }
    if (segObj.route !== undefined && !Array.isArray(segObj.route)) {
      throw new Error(`model-router: timeWindows.${seg}.route 须为数组`);
    }
  }
  // 规范化 + 冻结；route 数组冻结；checkProviderModel 在冻结前完成校验与字段提取
  const peakRoute = Array.isArray(tw.peak?.route) ? tw.peak.route : [];
  const valleyRoute = Array.isArray(tw.valley?.route) ? tw.valley.route : [];
  const normPeakRoute = peakRoute.map((hop, i) => checkProviderModel(hop, `timeWindows.peak.route[${i}]`));
  const normValleyRoute = valleyRoute.map((hop, i) => checkProviderModel(hop, `timeWindows.valley.route[${i}]`));
  return Object.freeze({
    enabled: tw.enabled === true,
    peakStart,
    valleyStart,
    peak: Object.freeze({ route: Object.freeze(normPeakRoute) }),
    valley: Object.freeze({ route: Object.freeze(normValleyRoute) }),
  });
}
...
export const DEFAULT_CONFIG = Object.freeze({
  ...
  timeZone: null,
  /**
   * v0.9.9 极简峰谷定价：两段完整候选链。
   * 未启用 / 缺字段 → router 链路零行为变化。
   * 详见 lib/router.js#segmentOf 与方案 §6-1。
   */
  timeWindows: Object.freeze({
    enabled: false,
    peakStart: '09:00',
    valleyStart: '22:00',
    peak: Object.freeze({ route: Object.freeze([]) }),
    valley: Object.freeze({ route: Object.freeze([]) }),
  }),
  /**
...
function checkProviderModel(hop, where) {
  ...
  // v0.9.10 Task 1：hop 内联 rpmLimit/tpmLimit/resetPolicy（同 providerMeta 段一样的语义）
  if (hop.rpmLimit !== undefined) {
    out.rpmLimit = validateRateLimit(hop.rpmLimit, 'rpmLimit', where).value;
  }
  if (hop.tpmLimit !== undefined) {
    out.tpmLimit = validateRateLimit(hop.tpmLimit, 'tpmLimit', where).value;
  }
  if (hop.resetPolicy !== undefined) {
    out.resetPolicy = validateResetPolicy(hop.resetPolicy, `${where}.resetPolicy`);
  }
  return out;
}
...
/**
 * v0.9.10 Task 1：限额声明校验（providerMeta.resetPolicy）。
 * 三种窗口：
 *   minute  — 每分钟滚动（无需 at/rollingSec）
 *   hour    — 每小时整点重置 → 必须给 at='HH:MM' 形式（与 hours.start 同正则）
 *   day     — 每日重置 → 必须给 at='HH:MM'
 *   rolling — 自定义滚动窗口秒 → 必须给 rollingSec=正整数（1-86400）
 *
 * 与既有 hours 不同：hours 用于"路由在时段 X-Y 用此渠道"，本函数用于
 * "上游限额在何时重置"——前者是主动选择，后者是被动窗口。
 */
function validateResetPolicy(rp, where) {
  if (!isPlainObject(rp)) {
    throw new Error(`model-router 配置错误：${where} 须为对象`);
  }
  const WINDOWS = ['minute', 'hour', 'day', 'rolling'];
  if (typeof rp.window !== 'string' || !WINDOWS.includes(rp.window)) {
    throw new Error(`model-router 配置错误：${where}.window 须为 ${WINDOWS.join(' | ')} 之一`);
  }
  const out = { window: rp.window };
  if (rp.window === 'hour' || rp.window === 'day') {
    if (typeof rp.at !== 'string' || !HHMM_RE.test(rp.at)) {
      throw new Error(`model-router 配置错误：${where}.at 须为 HH:MM 字符串（如 "02:00"，window=${rp.window} 时必填）`);
    }
    out.at = rp.at;
  }
  if (rp.window === 'rolling') {
    if (!Number.isInteger(rp.rollingSec) || rp.rollingSec < 1 || rp.rollingSec > 86400) {
      throw new Error(`model-router 配置错误：${where}.rollingSec 须为 1-86400 整数（window=rolling 时必填）`);
    }
    out.rollingSec = rp.rollingSec;
  }
  return out;
}

/** v0.9.10 Task 1：URL 校验（仅 http(s)://，拒绝 javascript:/data:/file: 等） */
function validateSourceUrl(url, where) {
  if (typeof url !== 'string' || !url) {
    throw new Error(`model-router 配置错误：${where} 须为非空字符串`);
  }
  if (!/^https?:\/\//i.test(url)) {
    throw new Error(`model-router 配置错误：${where} 须以 http(s):// 开头（拒绝 javascript:/data:/file: 等危险协议）`);
  }
  return url;
}

/**
 * v0.9.10 Task 1：RPM/TPM 上限校验。
 * 须为正整数；>10000 返回 `{ warn: '...' }` 让主流程决定是否打日志（避免隐式依赖外层 log）。
 * @returns {{value:number, warn?:string}} 校验通过；warn=undefined 表示无需警告
 */
function validateRateLimit(value, field, where) {
  if (!Number.isInteger(value) || value < 1) {
    throw new Error(`model-router 配置错误：${where}.${field} 须为正整数`);
  }
  if (value > 10000) {
    return { value, warn: `${where}.${field}=${value} 异常大——确认单位（每分钟请求数 vs 每天？每秒？）` };
  }
  return { value };
}
...
export function normalizeConfig(raw, opts = {}) {
  ...
  const merged = {
    ...
    // v0.9.9（方案 §6-1）：timeWindows 接入白名单；浅拷贝默认值（peak/valley.route
    // 后续按需解冻追加），未启用 / 缺字段由 router 链路零行为变化。
    timeWindows: normalizeTimeWindows(c.timeWindows),
  ...
  if (c.providerMeta !== undefined) {
    ...
    for (const [provider, m] of Object.entries(c.providerMeta)) {
      ...
      // v0.9.6：exclude —— 只裁剪「注册表自动展开」的候选池（registry.registeredPairs），
      // 不否决用户手工写进 rule.route 的条目、也不否决当前会话 seed。
      // 语义见 lib/registry.js isExcluded()。
      if (m.exclude !== undefined) {
        if (typeof m.exclude !== 'boolean') {
          throw new Error(`model-router 配置错误：providerMeta.${provider}.exclude 须为布尔值`);
        }
        entry.exclude = m.exclude;
      }
      // v0.9.10 Task 1：新增声明字段（声明优先，进路由决策）
      if (m.rpmLimit !== undefined) {
        const r = validateRateLimit(m.rpmLimit, 'rpmLimit', `providerMeta.${provider}`);
        entry.rpmLimit = r.value;
        if (r.warn) log?.warn?.(r.warn);
      }
      if (m.tpmLimit !== undefined) {
        const r = validateRateLimit(m.tpmLimit, 'tpmLimit', `providerMeta.${provider}`);
        entry.tpmLimit = r.value;
        if (r.warn) log?.warn?.(r.warn);
      }
      if (m.resetPolicy !== undefined) {
        entry.resetPolicy = validateResetPolicy(m.resetPolicy, `providerMeta.${provider}.resetPolicy`);
      }
      if (m.resetPolicySourceUrl !== undefined) {
        entry.resetPolicySourceUrl = validateSourceUrl(m.resetPolicySourceUrl, `providerMeta.${provider}.resetPolicySourceUrl`);
      }
      if (m.tpmLimitSourceUrl !== undefined) {
        entry.tpmLimitSourceUrl = validateSourceUrl(m.tpmLimitSourceUrl, `providerMeta.${provider}.tpmLimitSourceUrl`);
      }
      if (m.notes !== undefined) {
        if (typeof m.notes !== 'string') {
          throw new Error(`model-router 配置错误：providerMeta.${provider}.notes 须为字符串`);
        }
        if (m.notes.length > 500) {
          throw new Error(`model-router 配置错误：providerMeta.${provider}.notes 长度上限 500（防 DoS）`);
        }
        entry.notes = m.notes;
      }
...
export function normalizeState(raw, legacyMatch = false, opts = {}) {
  ...
  const normalized = normalizeConfig({
    ...
    // v0.9.9（方案 §6-1）：面板可提交 timeWindows 段。未提交时 undefined → 不重置。
    ...(raw.timeWindows !== undefined ? { timeWindows: raw.timeWindows } : {}),
  }, { log });
  return {
    ...
    // v0.9.9：回传 timeWindows（面板保存后立即生效）
    ...(raw.timeWindows !== undefined ? { timeWindows: normalized.timeWindows } : {}),
  };
}
