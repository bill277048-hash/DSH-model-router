...
export const DEFAULT_CONFIG = Object.freeze({
  ...
  sessionsDir: '/Users/apple/.deepseek-harness/sessions',
...
export function normalizeConfig(raw) {
  ...
  if (c.storePath !== undefined && c.storePath !== null) {
    if (typeof c.storePath !== 'string' || !c.storePath.startsWith('/')) {
      throw new Error('model-router 配置错误：storePath 须为绝对路径或 null');
    }
    merged.storePath = c.storePath;
  } else {
    // 默认落在 dsh home（本地 APFS，原子写安全）
    merged.storePath = DEFAULT_CONFIG.storePath ?? '/Users/apple/.deepseek-harness/home/model-router-state.json';
  }
  // 持久化会话根目录（v0.6.0 三级标题解析的第 3 级数据源）
  if (c.sessionsDir !== undefined && c.sessionsDir !== null) {
    if (typeof c.sessionsDir !== 'string' || !c.sessionsDir.startsWith('/')) {
