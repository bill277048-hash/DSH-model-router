...
import { writeFileSync, mkdirSync, renameSync, readdirSync, statSync, unlinkSync } from 'node:fs';
 ...
 *
 * v0.9.20 修复 I1：**失败时清理 tmp 文件**。此前只 `throw` 不清理，反复失败会在
 * 档案目录累积 `.tmp-*` 垃圾（且其中含报告内容）。对照 `lib/daily.js` 的同款实现
 * ——它一直有 `unlinkSync(tmp)` 清理，本模块 v0.9.9.2 统一原子写时**漏掉了这一步**，
 * 属重构引入的功能倒退。
 *
 * 清理失败仅忽略（`catch {}`）——不能因为清理失败掩盖真正的写盘错误。
 */
export function writeAtomic(target, content, log) {
  ...
  try {
    ...
    try {
      ...
      // tmp 可能根本没创建成功（writeFileSync 就失败）→ 忽略
    }
    throw error;
  }
}
 ...
 *
 * **实测依据**（v0.9.21 在实机 dsh 上直接计时，10 份 / 173KB）：
 * 本函数（只 stat）耗时 **1.5–2.4ms**；对照读全文的 `routes.listRunJson` 为 12–17ms。
 * 注：方案 §5 记的「读全文 466ms」是在带 I/O 拦截的环境测的，**虚高约 35 倍** ——
 * 轻量化的收益是真实的，但没有当时认为的那么悬殊（差 ~7 倍而非数量级）。
 *
 * 只扫 `ARCHIVE_KINDS` 白名单后缀，忽略同目录下的 `.ndjson`（日账本）/
 * `.report.md`（每日报告）等其他文件。
 *
 * @param {string} dir 档案目录
 * @returns {Array<{kind:string, runId:string, size:number, mtime:string}>} 按**写入时间**倒序
 *
 * v0.9.20 修复 I2：字段名由 `startedAt` 改为 **`mtime`**。
 * 此前叫 `startedAt` 但装的是**文件 mtime**，而详情视图（`routes.js`）的
 * `startedAt` 是**报告自身字段** —— 同名异义导致：列表按 mtime 排序（经
 * `POST /model-test/manual` 重写档案后 mtime 变新，旧跑批会「跳」到列表顶部），
 * 且点进详情后显示的时间与列表不同。
 *
 * 只 `stat` 不读内容，故拿不到报告自身的 `startedAt`（那是刻意取舍，见模块头注释）；
 * 需要精确开始时间时看 `runId`（其前缀即 `YYYY-MM-DDTHH-MM-SS`）或进详情视图。
 */
