...
export class Router {
  ...
  /**
   * v0.9.10 Task 2：按**声明**限速过滤候选 hop（OQ1 决策：声明优先，不读 observed）。
   *
   * **当前实现只做 RPM 节流**（数据已具备：metrics.recent 记请求计数）；
   * **TPM 节流待 Task 3**——需要 metrics 累计 token 用量，目前仅有请求计数。
   *
   * 判定规则（过去 60s 滚动窗口）：
   *   - `providerMeta[provider].rpmLimit`（或 hop 内联 `rpmLimit`）未声明 → 不节流
   ...
   *
   * @param {{provider:string, model:string, key?:string, rpmLimit?:number}[]} chain
  ...
  throttleByDeclared(chain) {
    ...
    // 取近 60s 每个 provider 的请求计数（不读 tpmLimit —— 缺 token 计数数据）
    let recent;
    try { recent = this.metrics.snapshot().recent ?? []; }
    catch { return chain; } // snapshot 抛错 → 不阻塞（容错优先）
    ...
    return chain.filter((hop) => {
      ...
      const declared = hop.rpmLimit !== undefined ? hop.rpmLimit : meta[hop.provider]?.rpmLimit;
      if (!Number.isInteger(declared) || declared < 1) return true; // 未声明 → 不过滤
      const count = counts.get(hop.provider) ?? 0;
      if (count >= declared * threshold) return false; // 跳过
      return true;
    });
  }
