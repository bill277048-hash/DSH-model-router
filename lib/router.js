...
export function localHHMM(timeZone, now = new Date()) {
  // v0.9.21 修复 S1：**必须显式 `hourCycle: 'h23'`**。
  //
  // 本函数返回值被用于**字符串比较**（`inTimeWindow` / `segmentOf` / `daily.tick`
  // 都依赖 "HH:MM" 零填充有序）。仅给 `hour12: false` 而不指定 `hourCycle` 时，
  // ICU 可能选 h24（1-24）而非 h23（0-23）→ 午夜产出 `"24:00"`，
  // 会破坏比较：如 peak 22:00 / valley 00:00 时，午夜会被误判为 peak。
  //
  // 当前 Node/V8 + en-GB 实测返回 "00:00"（无实际 bug），但插件分发到其他
  // Node/ICU 构建时不能保证 —— 显式 h23 把口径钉死，成本为零。
...
/**
 * 纯函数：由当地 `"HH:MM"` 判定峰/谷段。**半开区间 `[peakStart, valleyStart)`**。
 *
 * v0.9.21 抽出：此前 `router.segmentOf` 与 `routes.serializeTimeWindows` 各写一份
 * 同一判定（后者注释还写着「与 router.segmentOf 实现对齐」——靠人工对齐即漂移风险）。
 * 现统一走本函数，两处口径必然一致。
 *
 * 跨零点（`peakStart > valleyStart`，如 22:00→09:00）按
 * 「`>= peakStart` 或 `< valleyStart`」拼接成完整 24 小时，**无重叠无缝隙**。
 * `peakStart === valleyStart` 由 config 层校验拒绝，此处不重复防御。
 *
 * @param {string} hhmm 当地 "HH:MM"（须零填充，见 `localHHMM`）
 * @param {string} peakStart 峰段起点 "HH:MM"
 * @param {string} valleyStart 谷段起点 "HH:MM"
 * @returns {'peak'|'valley'}
 */
export function segmentOfHHMM(hhmm, peakStart, valleyStart) {
  const inPeak =
    (peakStart <= valleyStart)
      ? (hhmm >= peakStart && hhmm < valleyStart)
      : (hhmm >= peakStart || hhmm < valleyStart);
 ...
 *
 * ⚠ **端点语义与 `segmentOf` 刻意不同**：本函数**含**结束端点（`<=`），
 * 而 `segmentOf` 是半开区间（`<`）。即 t 恰为 valleyStart 时：
 * 规则时段窗 `[.., 22:00]` 仍命中，但段已切 valley。
 * 两者是独立特性（前者管「规则何时生效」，后者管「用哪条峰谷链」），
 * 各自语义合理；但**同时配置两者时，边界分钟会不一致** —— 故在此显式记录，
 * 避免后续维护者误以为是 bug 而「修」坏其中一个。
 */
...
export class Router {
  ...
  segmentOf(tw, tz, now = new Date()) {
    ...
    // v0.9.21：判定逻辑抽到 `segmentOfHHMM`（与 routes.serializeTimeWindows 共用，
    // 消除人工对齐带来的漂移风险）。
    return segmentOfHHMM(localHHMM(tz, now), peakStart, valleyStart);
  }
