...
/**
 * v0.9.10 Task 3c：从 usage 分片提取 token 总数（**唯一实现**）。
 *
 * 口径（宽松取数，与既有 `QuotaLedger.record` 一致）：
 * `inputTokens + outputTokens + cacheReadTokens + cacheWriteTokens`，
 * 四项全为 0/缺失时退回 `totalTokens`。
 *
 * @param {object} usage usage 分片（形如 {inputTokens, outputTokens, ...}）
 * @returns {number} token 总数（>=0；非法/缺失 → 0）
 */
export function usageTokens(usage) {
  ...
  const tokens = sum || (Number(usage.totalTokens) || 0);
