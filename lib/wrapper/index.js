...
import { usageTokens } from '../quota.js';
import { PROBE_MARK } from '../probe.js';
import { MODE_PRESETS } from '../modes.js';
import { deriveWindowId } from '../quota.js';

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

/** §11：触发 markBurnedOut 的上游错误码（QUOTA/RATE_LIMIT 类）。 */
const QUOTA_SIGNALS = new Set(['QUOTA', 'QUOTA_EXCEEDED', 'RATE_LIMIT']);

/**
 * v0.9.10 Task 3a：把上游错误码细分成方向性方案 v1.3 §4.7 提出的三类。
 *
 * **不**改既有 failover / cooldown 路径（仍以原始 code 为准）—— 只在
 * `metrics.sample({errorCode})` 之前做一次"细分重命名"，驱动 `observed.*Count` 计数。
 * 失败/不属于三类的 code → 原样返回（保留可观测性）。
 *
 * 三类（驱动 v0.9.10 §6 冲突阈值）：
 * - RATE_LIMITED  —— 速率限制（429001 / "inference exceeds tpm/rpm"）
 * - QUOTA_EXHAUSTED —— 配额耗尽（quota_exceeded_error，**非账号级**）
 * - ACCOUNT_TPM_LIMITED —— 账号级 TPM（ModelAccountTpmRateLimitExceeded，**换模型无效**）
 *
 * 模式匹配容错：DSH 适配器各 upstream code 大小写/拼写可能略差异，用子串 + lowercase 兜底。
 */
export function classifyBurnError(rawCode) {
  if (typeof rawCode !== 'string' || !rawCode) return { code: rawCode, kind: 'other' };
  const c = rawCode.toUpperCase();
  // 账号级 TPM 必须先匹配（含 'ACCOUNT' 字样）—— 优先级高于通用 RATE_LIMIT
  if (c.includes('ACCOUNT_TPM') || c.includes('ACCOUNTTPM') || c === 'ACCOUNT_TPM_RATE_LIMIT_EXCEEDED' ||
      c.includes('ACCOUNT TPM') || c.includes('ACCOUNT-TPM')) {
    return { code: 'ACCOUNT_TPM_LIMITED', kind: 'accountTpm' };
  }
  if (c === 'RATE_LIMIT' || c === 'RATE_LIMITED' || c.includes('429001') ||
      c.includes('INFERENCE EXCEEDS TPM') || c.includes('INFERENCE EXCEEDS RPM') ||
      c.includes('TPM RATE LIMIT') || c.includes('RPM RATE LIMIT')) {
    return { code: 'RATE_LIMITED', kind: 'rateLimited' };
  }
  if (c === 'QUOTA' || c === 'QUOTA_EXCEEDED' || c === 'QUOTA_EXHAUSTED' ||
      c.includes('QUOTA_EXCEEDED_ERROR') || c.includes('QUOTA EXCEEDED')) {
    return { code: 'QUOTA_EXHAUSTED', kind: 'quotaExhausted' };
  }
  // 不在三类 → 原样返回
  return { code: rawCode, kind: 'other' };
}

/**
 * v0.9.4 B-1：跨请求节流——距「该 provider 上次成功发送」尚需等待的毫秒（纯函数）。
 * @param {Map<string, number>} ledger provider → 最近一次成功发送时刻 ms（S-1：
 *   提交只发生在成功路径，本函数纯读不 set，避免并发预占混乱）
 * @param {string} target provider id
 * @param {number} intervalMs 请求间隔毫秒（free-tier preset requestIntervalMs）
 * @param {number} [now] 当前时刻毫秒（单测注入；缺省 Date.now()）
 * @returns {number} 等待毫秒（>=0）；intervalMs<=0 → 0（不节流）；从未成功发送 → 0
 */
export function throttleWait(ledger, target, intervalMs, now = Date.now()) {
  if (!(intervalMs > 0)) return 0;
  const last = ledger.get(target) ?? 0;
...
/**
 * F-2（台阶②）：per-request mode 参数解析（纯函数，便于单测）。根据命中 rule 的
 * mode 查预设，返回本次请求应覆盖的时序参数局部值；rule 无 mode / 预设无对应字段
 * 时返回 undefined（调用方沿全局）。不写全局 config、不涉及 cooldown/maxRetries。
 * @param {object|null} matchedRule router.match 的命中规则
 * @returns {{ budget: (number|undefined), watchdog: (number|undefined) }}
 */
export function resolveRequestMode(matchedRule) {
  if (!matchedRule) return { budget: undefined, watchdog: undefined, custom: null, requestIntervalMs: undefined, interModelDelayMs: undefined };
  // v0.9（自定义模式）：custom 规则直接用 rule.custom 的预算/看门狗/冷却；
  // 未填的键回退 undefined，调用方沿全局 config / fallbackPolicy。
  if (matchedRule.mode === 'custom') {
    const c = matchedRule.custom || {};
    return {
      budget: c.failoverBudgetMs,
      watchdog: c.firstTokenTimeoutMs,
      custom: {
        cooldownSec: c.cooldownSec,
        quotaCooldownSec: c.quotaCooldownSec,
      },
      requestIntervalMs: undefined, // custom 不消费
      interModelDelayMs: undefined, // custom 不消费
    };
  }
  const preset = matchedRule.mode ? MODE_PRESETS[matchedRule.mode] : null;
  return {
    budget: preset?.failoverBudgetMs,
    watchdog: preset?.firstTokenTimeoutMs,
    custom: null,
    requestIntervalMs: preset?.requestIntervalMs,
    interModelDelayMs: preset?.interModelDelayMs,
  };
}
...
export function createStreamWrapper({ config, router, cooldown, metrics, quota, daily, log }) {
  ...
  let seqCounter = 0;
  /**
   * v0.9.9（方案 §6-1 第 6 条）：当前段。wrapper 顶层取一次，passthrough/runAttempt/
   * allBlocked 三类采样共用。未启用 → null（与 router.segmentOfNow 同口径）。
   * 性能：每调用算一次 Intl.DateTimeFormat，可接受（wrapper 每秒最多几十次调用）。
   */
  const segNow = () => (typeof router.segmentOfNow === 'function' ? router.segmentOfNow() : null);
  /** v0.9.4 B-1：跨请求节流账本（provider → 最近一次成功发送时刻 ms；S-1 成功路径提交）。 */
  const interProvider = new Map();
  /** v0.9.4 B-1：跨请求节流等待毫秒（纯读，不 set——并发预占混乱；
   *  提交统一在成功路径）。实现见模块级纯函数 throttleWait。 */
  const stats = {
    wraps: 0,
    passthroughs: 0,
    failovers: 0,
  ...
  // §11：provider 的窗口声明（providerMeta），供 consumeAttempt/markBurnedOut。
  const providerMetaFor = (provider) =>
    (config?.providerMeta && config.providerMeta[provider]) || {};
  const gateBlocked = (provider) => {
    if (!quota || typeof quota.consumeAttempt !== 'function') return null;
    return quota.consumeAttempt(provider, providerMetaFor(provider));
  };
  const maybeBurn = (provider, code) => {
    if (!quota || typeof quota.markBurnedOut !== 'function') return;
    if (!code || !QUOTA_SIGNALS.has(String(code))) return;
    try {
      quota.markBurnedOut(provider, deriveWindowId(code), providerMetaFor(provider));
    } catch {
      // markBurnedOut 失败不阻断 failover 主流程
    }
  };
  ...
  async function* passthrough(options, next, seed, seqId) {
    ...
    // §11：透传路径同样受窗口限额 gate（不消耗 token、不打上游）。
    const gate = gateBlocked(seed.provider);
    if (gate && !gate.allowed) {
      stats.passthroughs += 1;
      stats.quotaGated = (stats.quotaGated || 0) + 1;
      // v0.9.8：让「切换明细」能看到被 gate 拦截的请求——否则 stats 计数与 metrics 完全脱节。
      metrics.sample({
        sessionId: seed.sessionId, provider: seed.provider, model: seed.model,
        attemptIndex: 0, outcome: 'failed', errorCode: 'QUOTA',
        e2eMs: Date.now() - startedAt, seqId,
        prevProvider: null, prevModel: null, switched: false,
        segment: segNow(),
      });
      yield {
        type: 'finish',
        reason: {
          ...
          failure: {
            code: 'QUOTA',
            message: `model-router: window quota exhausted (${gate.reason}, reset ${gate.resetAt ?? 'n/a'})`,
          },
        },
      };
      return;
    ...
    try {
      ...
      // v0.9.8：透传流抛异常也要进入切换明细；正常路径/QUOTA gate 各自已经提前
      // 采样一次，这里只覆盖 throw 分支，避免重复。
      // v0.9.10 Task 3a：errorCode 经 classifyBurnError 细分（驱动 observed 计数）
      metrics.sample({
        ...
        attemptIndex: 0,
        outcome: 'failed',
        errorCode: classifyBurnError(error?.code ?? 'STREAM_ERROR').code,
    ...
    // v0.9.4 B-1（S-1）：透传成功（无 error finish）也推进跨请求节流账本——
    // free-tier 无候选直连场景同样受跨请求间隔约束。
    if (recOutcome !== 'failed') interProvider.set(seed.provider, Date.now());
    // v0.9.8：透传路径采 1 条 metrics（无候选直连场景下若不采样，切换明细整张表为空，
    // 看似「无切换」实际是「数据未采集」）。这是一个独立请求，prev* 明确为 null。
    metrics.sample({
      sessionId: seed.sessionId, provider: seed.provider, model: seed.model,
      attemptIndex: 0, outcome: recOutcome,
      errorCode: recOutcome === 'failed' ? classifyBurnError(recErrorCode).code : recErrorCode,
      e2eMs: Date.now() - startedAt, seqId,
      prevProvider: null, prevModel: null,
      switched: false,
      segment: segNow(),
    });
    if (daily) {
  ...
  async function* runAttempt({
    ...
    // v0.9.8：把前序 hop 抽象成局部变量，7 处 metrics.sample 统一补字段。
    // switched 由真实前序与当前 provider/model 比较得出，不用 attemptIndex 猜。
    const prevP = prevProvider ?? null;
    const prevM = prevModel ?? null;
    // v0.9.10 Task 3c：声明 lastUsage 占位（闭包：metaFields.spread 发生在各 sample 时，
    // 此时 lastUsage 已被 for-await 填充；spread 读最新值）
    let lastUsage = null;
    // v0.9.9：把 segment 也并入 metaFields，7 处采样自动标注所属段。
    // v0.9.10 Task 3c：tokens 也并入（lastUsage）；spread 时自动取最新值
    const metaFields = {
      seqId,
      prevProvider: prevP,
      prevModel: prevM,
      switched: prevP !== null && (prevP !== provider || prevM !== model),
      segment: segment ?? null,
      // v0.9.10 Task 3c：usage 分片 → token 总数（复用 quota.usageTokens 唯一口径）；
      // 无 usage / 全 0 → null（metrics 侧只累计数字）
      get tokens() { return lastUsage ? (usageTokens(lastUsage) || null) : null; },
    };
    const startedAt = Date.now();
    let recOutcome = 'committed';
    let recErrorCode;
      ...
      while (true) {
        ...
        if (raced === TIMEOUT) {
          ...
          metrics.sample({
            ...
            errorCode: 'TIMEOUT', e2eMs: Date.now() - startedAt, ...metaFields,
          });
        ...
        if (done) {
          ...
          metrics.sample({
            sessionId: state.sessionId, provider, model, attemptIndex, outcome: 'failed',
            errorCode: 'EMPTY_RESPONSE', e2eMs: Date.now() - startedAt, ...metaFields,
          });
          if (!isLast) {
        ...
        if (chunk.type === 'finish') {
          ...
          if (r.kind === 'aborted') {
            ...
            stats.userAborts += 1;
            metrics.sample({
              sessionId: state.sessionId, provider, model, attemptIndex, outcome: 'aborted', e2eMs: Date.now() - startedAt, ...metaFields,
            });
            if (!state.committed) yield* flushPending(state);
            state.committed = true;
            yield chunk;
          ...
          if (r.kind === 'error') {
            ...
            if (!state.committed && signals.has(code) && !isLast) {
              ...
              // v0.9.4 B-5：context 类失败打点——router 侧 60s 内 contextReorder
              // 把大窗口候选前移（判定本身不变，只追加打点）。
              ...
              maybeBurn(provider, code);
              metrics.sample({
                sessionId: state.sessionId, provider, model, attemptIndex, outcome: 'failed',
                // v0.9.10 Task 3a：上游 code 细分（驱动 observed.*Count）
                errorCode: classifyBurnError(code).code, e2eMs: Date.now() - startedAt, ...metaFields,
              });
              cleanupStream(stream);
            ...
            metrics.sample({
              ...
              // v0.9.10 Task 3a：失败时细分 errorCode（成功时原样）
              errorCode: state.committed ? code : classifyBurnError(code).code,
              e2eMs: Date.now() - startedAt, ...metaFields,
            });
            if (state.committed) {
              cooldown.onFail(hopKeyOf({ provider, model }), code);
              maybeBurn(provider, code);
          ...
          // v0.9.4 B-1（S-1）：成功 finish 才推进跨请求节流账本——失败/中止不推进，
          // 避免「一次失败锁死 provider N 秒」。
          interProvider.set(provider, Date.now());
          metrics.sample({
            sessionId: state.sessionId, provider, model, attemptIndex, outcome: 'committed',
            ttftMs: state.ttftMs, e2eMs: Date.now() - startedAt, ...metaFields,
          });
          if (!state.committed) yield* flushPending(state);
          state.committed = true;
          yield chunk;
  ...
  async function* streamWrapper(options, next) {
    ...
    // F-2（台阶②）+ F-1（批 3）：首先解析「绑定规则」——请求若携带 __mr_rule
    //（agent/request 会话级提议附加，绑定用户选中的规则），用 byName 取回该规则，
    // 避免用被替换后的真实 provider 重新 match 命中 default（F-1 根因）。无 __mr_rule
    // 时回落 seed 匹配（零行为变更）。
    const bindRule =
      typeof options.__mr_rule === 'string' ? router.byName(options.__mr_rule) : null;
    const matchedRule = bindRule ?? router.match(seed);
    const { budget: reqBudget, watchdog: reqWatchdog, custom: reqCustom, requestIntervalMs: reqIntervalMs, interModelDelayMs: reqInterModelDelayMs } = resolveRequestMode(matchedRule);
    // v0.9.4 B-1：free-tier 跨请求节流——发起前对 seed.provider 只读 throttleWait
    // （纯读不 set；S-1 定稿：成功路径才提交 interProvider）。仅 reqIntervalMs>0
    // 生效；普通/其它模式 reqIntervalMs===undefined → 零行为变化。
    const seedWait = !(reqIntervalMs > 0) ? 0 : throttleWait(interProvider, seed.provider, reqIntervalMs);
    if (seedWait > 0) {
      log.info(`free-tier throttle ${seed.provider} sleep ${seedWait}ms`);
      await sleep(seedWait);
    }
    // v0.9（自定义模式）：绑定 custom 规则时，把其 cooldown/quotaCooldown 作为
    // per-request 覆盖传入 cooldown.allow（共享 hop 的期满判定「最近写入的 rule 胜出」）。
    const cooldownOverride = reqCustom &&
      (reqCustom.cooldownSec !== undefined || reqCustom.quotaCooldownSec !== undefined)
      ? { cooldownSec: reqCustom.cooldownSec, quotaCooldownSec: reqCustom.quotaCooldownSec }
      : undefined;
    // v0.8.0 G1：调用序列标识（attempt 记账分组用；透传与切换路径共用）
    const seqId = `${Date.now()}-${++seqCounter}`;
    // candidates() 内部已按「链耗尽时间窗」收敛为单候选（R-2/R-8 汇聚点）。
    // F-1：绑定规则存在时，候选链以绑定规则生成（避免命中 default 的空/错误链）。
    const chain = bindRule ? router.candidatesForRule(bindRule, seed) : router.candidates(seed);
    ...
    let usable = failoverCandidates.filter((hop) => cooldown.allow(hopKeyOf(hop), cooldownOverride));
    ...
    const primaryUsable = cooldown.allow(hopKeyOf(seed), cooldownOverride);
    ...
    // §11 入口拦截：跳过已耗尽窗口的 provider（候选链 + 首选一并过滤），
    // 窗口耗尽时主动避让，不打实际 API；全部被 gate → 合成 QUOTA finish。
    if (quota && typeof quota.consumeAttempt === 'function') {
      const filtered = [];
      let allBlocked = true;
      for (const a of seq) {
        const gate = quota.consumeAttempt(a.provider, providerMetaFor(a.provider));
        if (gate && !gate.allowed) {
          stats.quotaGated = (stats.quotaGated || 0) + 1;
          log.info(`quota gate ${a.provider}/${a.model} skip: ${gate.reason} msToReset=${gate.msToReset ?? 'n/a'}`);
          continue;
        }
        allBlocked = false;
        filtered.push(a);
      ...
      if (allBlocked) {
        stats.exhaustions += 1;
        router.recordExhausted({ code: 'QUOTA', message: 'model-router: all window quotas exhausted' });
        // v0.9.8：入口 quota gate 直接合成 finish 时没有经过 passthrough/runAttempt，
        // 这里单独采一条失败记录，避免「切换统计有链耗尽但明细为空」；不调用
        // quota.record/consumeAttempt 第二次，不会产生重复配额记账。
        metrics.sample({
          sessionId: state.sessionId,
          provider: seed.provider,
          model: seed.model,
        ...
        yield {
          ...
          reason: {
            kind: 'error',
            failure: { code: 'QUOTA', message: 'model-router: all window quotas exhausted' },
          },
        };
        return;
      }
    ...
    let budgetLeft = reqBudget ?? config.failoverBudgetMs ?? 90_000;
    // 下限仅防 0/负值（生产下限由 normalizeConfig >=1000 校验保证；测试用小值不被钳）
    const watchdogMs = () => Math.max(1, Math.min(reqWatchdog ?? config.firstTokenTimeoutMs, budgetLeft));

    let outcome;
    let prevAttemptStart = null;
    for (let i = 0; i < seq.length; i++) {
      const a = seq[i];
      const isLast = i === seq.length - 1;
      // v0.9.8：段链路需要前序 hop 信息（prevProvider/prevModel），按方案 §5-1 在循环入口
      // 算一次。首跳的 prev 是请求种子（wrapper 入口的 seed），后续跳的 prev 是上
      // 一跳的实际 hop。已与 wrapper/index.js:664 既有「上一跳 provider」用法一致。
      const prevHop = i > 0 ? seq[i - 1] : { provider: seed.provider, model: seed.model };
      // v0.9.3 P1-7：free-tier 预设消费——相邻请求最小间隔 + 切换 provider 额外延迟
      // v0.9.4 B-1：口径统一——保留请求内相邻间隔（elapsed 基准），叠加 provider 级
      // 跨请求节流（throttleWait，纯读同源账本；新 provider 未提交过 → 0 不干预）。
      if (i > 0 && (reqIntervalMs > 0 || reqInterModelDelayMs > 0)) {
        const elapsed = Date.now() - prevAttemptStart;
        if (reqIntervalMs > 0) {
          const wait = Math.max(reqIntervalMs - elapsed, throttleWait(interProvider, a.provider, reqIntervalMs));
          if (wait > 0) await sleep(wait);
        }
        if (reqInterModelDelayMs > 0 && a.provider !== seq[i - 1].provider) {
          await sleep(reqInterModelDelayMs);
        }
      }
      ...
      try {
        if (a.viaNext) {
          ...
          outcome = yield* runAttempt({
            ...
            prevProvider: prevHop.provider,
            prevModel: prevHop.model,
            segment: segNow(),
          });
        } else {
