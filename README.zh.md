...
DSH 多供应商模型路由插件（v0.9.4）：规则路由 + 首 token 前无感故障切换 + cooldown 熔断 + 用量记账 + **每日 API 报告** + **面板信息分级** + **Windows 适配** + 状态接口 + WebUI 面板 + **match 收紧密（allowLegacyMatch）** + **free-tier 跨请求节流** + **context 窗口感知重排** + **每日报告一键启用与生成 toast**。
...
- **路径可见**：`/status` 顶层回显 `reports: { enabled, reportDir, hour }`（默认报告目录 `~/.deepseek-harness/home/reports/`），面板已启用态顶部直接显示
...
| `reports` | `{enabled:false, hour:'01:00'}` | v0.8.0 G1 每日报告开关；`enabled:true` 后日账本记账 + 每日 `hour` 生成昨日报告；v0.9.4 起面板「每日报告」页签可一键开启（`POST /state`），`/status` 回显 `reports.enabled/reportDir/hour` |
