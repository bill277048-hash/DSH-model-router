...
**门禁自身的假阳性（2026-09-21 首次上线即踩到）**：条目级比对最初用
`unzip -l` 的 `大小 + 日期 + 名称` 三列，其中**日期列是源文件的 mtime** ——
CI 检出时 mtime 全变成检出时刻，本地打包用的是编辑时刻，**必然不一致**。
于是 tag `v1.0.2` 的首次发布被**误判**为「条目清单不一致」而失败。

已改为**只比「未压缩大小 + 条目名」**，真实内容差异由内容级 `diff -r` 兜底。
修正后双向验证：① mtime 不同但内容相同 → **pass**；② 注入一处内容差异 → **fail**。

> 与本报告 §7.2 记录的两处「校验器假阳性」同源：**未经校验的校验器会制造假缺陷，
> 比不校验更危险**。判据必须避开与被测对象无关的变量（此处是 mtime）。
