/**
 * @botton/dsh-model-router 插件浏览器半（client）v1.0.2。
 *
 * 手写 ModuleLoader 包装格式（无构建链、无 JSX，与 @botton/dsh-guardian 同款）：
 * 注册到 "web-ui.plugin.item" slot（WebUI 插件管理页卡片列表）。
 *
 * 面板结构（v1.0.2：**8 页签**三级）：
 * - L1 概览摘要卡：健康总览 / 今日运行 / 昨日报告 / 熔断速览 / 规则速览；
 *   概览态 5s 轮询 `?l1=1` 轻量摘要接口，明细页签才拉全量 status。
 * - L2 明细页签（**8 个**）：概览 / 切换规则 / 切换日志 / 诊断 / 可切换模型 /
 *   每日报告 / 模型测试 / 测试档案（v0.9.9.5 Task 7 起统一三类 kind 罗列 + 详情）。
 * - L3 工具折叠区（4 块）：健康探测 / TRM 压测 / 用量窗口 / 窗口限额（v0.9.7）。
 *
 * 要点：
 * - 切换规则：命名「规则包」（v0.9.1 纯选择驱动）+ 每条规则独立优先模式 +
 *   hop「⚙ 高级」内联声明（v0.9.19）。
 * - 诊断：按 `seqId` 分组的切换明细（含段标注）。
 * - 每日报告：近 30 天日期下拉 + 结构化 / Markdown 双视图 + 一键启用与生成。
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // v0.8.0 G2 + v0.9.8 七页签 → v0.9.9.5 起八页签（+ 测试档案）
    var TABS = [
